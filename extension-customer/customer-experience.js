...
(() => {
  ...
  async function synchronizeProject() {
    ...
    try {
      ...
      const cloudflareReady = Boolean(statusData.ai?.cloudflare?.configured);

      if (!cloudflareReady) {
        setHelpfulStatus("Configure a Cloudflare para liberar o uso. Gemini e OpenRouter continuam disponíveis como contingências opcionais.");
        return;
      }
      ...
      if (!desiredRepository) {
        if (!connectedRepository) {
          setHelpfulStatus("GitHub conectado. Conecte este projeto ao GitHub no Lovable; se o vínculo não for detectado, escolha o repositório uma única vez.");
        }
        return;
      }
      ...
      // Automação nunca deve bloquear o fluxo manual existente.
  ...
  document.addEventListener("superlovable:github-status", () => {
    setTimeout(() => void synchronizeProject(), 250);
  });

...
const USER_AGENT = "SuperLovable-OpenRouter";
const OPENROUTER_TIMEOUT_MS = 25_000;
const MAX_CONTEXT_FILES = 5;
const MAX_CONTEXT_CHARS = 30_000;

type AgentAuth = { license: { id: string } };
type OpenRouterCredential = { apiKey: string; model: string };
...
async function selectedContext(
  ...
  const candidates = [...repositoryPaths]
    .sort((a, b) => scorePath(b, prompt) - scorePath(a, prompt))
    .slice(0, 18);
  ...
  for (const path of candidates) {
    ...
    try {
      ...
      // Um arquivo individual indisponível não deve impedir o fallback inteiro.
    }
  }
...
function extractJson(raw: string) {
  ...
  try {
    ...
    throw new Response("O OpenRouter não retornou um plano de código válido.", { status: 502 });
  }
}
...
async function callOpenRouter(
  ...
  try {
    const model = credential.model || "openrouter/free";
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${credential.apiKey}`,
        "Content-Type": "application/json",
        "HTTP-Referer": "https://painel-super-lov.lovable.app",
        "X-Title": "Super Lovable",
      },
      body: JSON.stringify({
        model,
        temperature: 0.1,
    ...
    if (response.status === 401 || response.status === 403) {
      throw new Response(
        "A chave OpenRouter salva não foi aceita. A conexão foi mantida; substitua a API quando puder.",
        { status: 401 },
      );
    }
    if (response.status === 429) {
      throw new Response(
        "O OpenRouter atingiu o limite temporário. Sua chave continua salva; aguarde a cota renovar ou adicione outra API de contingência.",
        { status: 429 },
      );
    }
    if (!response.ok) {
      throw new Response(`OpenRouter indisponível no momento (${response.status}).`, {
    ...
    if (controller.signal.aborted) {
      throw new Response(
        "O OpenRouter demorou além do limite desta tentativa. A chave continua conectada.",
        { status: 504 },
      );
    }
    throw new Response("Não foi possível comunicar com o OpenRouter nesta tentativa.", { status: 503 });
  } finally {
...
async function materializePlan(
  ...
  for (const edit of edits) {
    const actualPath = repositoryPaths.find(
      (candidate) => candidate.toLowerCase() === edit.path.toLowerCase(),
    );
  ...
  for (const [path, pathEdits] of byPath) {
    ...
    for (const edit of pathEdits) {
      ...
      if (first < 0 || first !== last) {
        throw new Response(
          `O OpenRouter não encontrou um trecho único em ${path}. Tente novamente para uma nova leitura do projeto.`,
          { status: 422 },
        );
      }
...
export async function planAgentRunOpenRouter(
  ...
  if (!installationId || !repo) {
    throw new Response("Conecte e selecione o projeto GitHub primeiro.", { status: 422 });
  }
  ...
  const payload = JSON.stringify({
    ...
    repository: repo,
    branch,
    available_files: context.repositoryPaths.slice(0, 1_200),
    files: context.files,
  });
  const ai = await callOpenRouter(payload, credential);
  const parsed = extractJson(ai.raw);
  const files = await materializePlan(parsed, token, repo, branch, context.repositoryPaths);
  if (!files.length) {
    throw new Response(
      String(parsed.summary || "O OpenRouter não propôs uma alteração segura nesta tentativa."),
      { status: 422 },
    );
  }

  const summary = String(parsed.summary || "Alteração preparada pelo OpenRouter.").slice(0, 2_000);
  ...
  const { data, error } = await supabaseAdmin
    ...
    .single();
  if (error || !data) {
    throw new Response("Não foi possível salvar o plano gerado pelo OpenRouter.", { status: 500 });
  }
  return {

...
export const Route = createFileRoute("/api/public/agent/github/repositories")({
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        try {
          ...
          return json({
            ok: true,
            repositories: await agent.repositoriesForLicense(auth.license.id),
          });
        } catch (error) {
          ...
          return json(
            { ok: false, error: error instanceof Error ? error.message : "Falha ao listar projetos." },
            500,
          );
        }
      },
      POST: async ({ request }) => {
        ...
        try {
          ...
          return json(
            {
              ok: false,
              error: error instanceof Error ? error.message : "Falha ao selecionar projeto.",
            },
            500,
          );
        }

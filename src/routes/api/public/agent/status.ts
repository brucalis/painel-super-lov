  ...
  server: { handlers: {
    ...
    GET: async ({ request }) => {
      ...
      try {
        ...
        return json({ ok: true, configured: Boolean(process.env.GITHUB_APP_ID && process.env.GITHUB_CLIENT_ID && process.env.GITHUB_PRIVATE_KEY), ai: { gemini: Boolean(process.env.GEMINI_API_KEY), groq: Boolean(process.env.GROQ_API_KEY) }, connection });
      } catch (error) {

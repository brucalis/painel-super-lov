    ...
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        try {
          ...
          if (!credentials.isCustomerEdition(request))
            return json({ ok: false, error: "Recurso disponível somente na edição do cliente." }, 404);
          ...
          return json({ ok: false, error: "Não foi possível consultar a credencial." }, 500);
        }
      },
      PUT: async ({ request }) => {
        const { json } = await import("@/lib/license.server");
        try {
          ...
          return json({ ok: true, ...(await credentials.saveCustomerOpenAiKey(auth.license.id, body.api_key || "")) });
        } catch (error) {
          if (error instanceof Response) return json({ ok: false, error: await error.text() }, error.status);
      ...
      DELETE: async ({ request }) => {
        ...
        try {
          ...
          if (!credentials.isCustomerEdition(request))
            return json({ ok: false, error: "Recurso disponível somente na edição do cliente." }, 404);
          await credentials.deleteCustomerOpenAiKey(auth.license.id);
